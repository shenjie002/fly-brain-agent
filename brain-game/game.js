...
const LEVELS = {
  ...
  AI:     { name: 'Laya Brain 🧠',       chip: '#ff9800' },
};
const HELP = {
  ...
  AI: '🧠 Laya 快脑 AI 自主决策 → LIF 神经放电 → CPG 六足驱动' +
       '<div class="row2">Switch: <kbd>1</kbd>/<kbd>2</kbd>/<kbd>3</kbd>/<kbd>4</kbd> · Restart: <kbd>Space</kbd></div>',
};
...
const HELP_PAD = {
  ...
  AI: '🧠 AI is in control — watch the neural panel',
};

// --- AI Brain: Laya→LIF→CPG closed loop -----------------------------------
const PIPELINE_URL = 'http://localhost:8000';
const AI_DECIDE_INTERVAL_MS = 800; // how often to call /decide

// 13 motor neuron labels matching motor_map.py
const NEURON_LABELS = [
...
// Stateless fetch wrapper around the pipeline's POST /decide endpoint. The
// Game drives it (throttling + applying the result); this class only knows how
// to turn one state string into one decision.
class AIBrain {
  constructor() { this.lastError = null; }

  // Returns the parsed /decide response, or null on failure. Adds a measured
  // round-trip latency (ms) as `client_latency_ms`.
  async decide(state) {
    try {
      ...
      j.client_latency_ms = Math.round(performance.now() - t0);
      this.lastError = null;
      return j;
    } catch (e) {
...
class Game {
  constructor(mj, model, data, meta) {
    ...
    this._padState = null;
    this.ai = new AIBrain();
    this._aiGainL = 0; this._aiGainR = 0;   // CPG gains from the latest decision
    this._aiBusy = false; this._aiLastCall = 0;
    this._aiDecision = null;
  ...
  _applyLevelColor() {
    const lvKey = this.level === 'AI' ? 'CPG' : this.level;
    const c = this.meta.arena.level_ground_colors[lvKey] || [0.5, 0.5, 0.5, 1];
  ...
  _resetSim() {
    ...
    // Clear any stale AI decision so a restart begins from a full stop.
    this._aiGainL = 0; this._aiGainR = 0;
    this._aiBusy = false; this._aiLastCall = 0;
  ...
  setLevel(level) {
    ...
    this._applyLevelColor();
    this._renderBest();
    // Show/hide neural HUD panel (only visible in AI mode)
    const nh = document.getElementById('neural-hud');
    if (nh) nh.classList.toggle('visible', level === 'AI');
    this.restart();
  }
  ...
  _levelBlurb() {
    return {
      ...
      AI: 'Laya fast-brain reads the fly state, fires 13 motor neurons (LIF), and drives the CPG. Make sure pipeline.py is running on :8000. Sit back and watch the neural circuit work.',
    }[this.level];
  }
  ...
  _physicsStep() {
    ...
    if (this.level === 'AI') {
      // AI mode: use gains from the latest Laya decision
      this.controller.stepCPG(ctrl, this._aiGainL, this._aiGainR);
  ...
  _frame(nowMs) {
    ...
    if (this.phase === 'running') {
      // AI mode: periodically request a new decision from the backend
      if (this.level === 'AI') this._aiTick(now);
  ...
  _aiTick(now) {
    if (this._aiBusy || now - this._aiLastCall < AI_DECIDE_INTERVAL_MS / 1000) return;
    this._aiBusy = true;
    this._aiLastCall = now;
    const state = this._buildStateStr();
    this.ai.decide(state).then((res) => {
      this._aiBusy = false;
      if (!res) return;
      this._aiDecision = res;
      // Map Laya choice → CPG gains. Choice picks the pattern; confidence only
      // modulates strength (0.65 + 0.35*conf), matching motor_map.py.
      const g = 0.65 + 0.35 * (res.laya.confidence || 0.5);
      const map = {
        WALK:       [1, 1],
        FLAP:       [1.2, 1.2],
        HOLD:       [0, 0],
      ...
      const [bL, bR] = map[res.laya.choice] || [1, 1];
      this._aiGainL = bL * g;
      this._aiGainR = bR * g;
      this._updateNeuralHud(res);
    });
  }

  // Laya is an English natural-language model (numeric state strings collapse
  // confidence to <0.1, per DESIGN_NOTES). So describe the fly's situation as a
  // plain-English sentence that maps cleanly onto WALK / TURN_LEFT / TURN_RIGHT.
  _buildStateStr() {
    const b = this.bodyId, d = this.data;
    const x = d.xpos[3 * b], y = d.xpos[3 * b + 1];
    const speed = Math.hypot(d.cvel[6 * b + 3], d.cvel[6 * b + 4]);
    // The corridor to the finish runs along +x; its centre in y is the finish
    // line's midpoint. Being off-centre in y means the fly should steer back.
    const yTarget = (this.finish[0][1] + this.finish[1][1]) / 2;
    const offset = y - yTarget;                        // +y is to the fly's left
    const dist = Math.max(0, this.finish[0][0] - x);   // remaining distance in x

    if (dist < 1.5) return 'The fly has almost reached the goal, keep walking straight forward.';
    if (offset > 0.8) return 'The fly has drifted to the left of the path, it should turn right to steer back toward the goal.';
    if (offset < -0.8) return 'The fly has drifted to the right of the path, it should turn left to steer back toward the goal.';
    if (speed < 0.05) return 'The fly is standing still on the ground and needs to start walking forward toward the goal.';
    return 'The path ahead is clear, keep walking straight forward toward the goal.';
  }

  _updateNeuralHud(res) {
    const panel = document.getElementById('neural-hud');
    if (!panel) return;
    panel.classList.add('visible');
    // Decision + confidence
    const choiceEl = document.getElementById('ai-choice');
    if (choiceEl) choiceEl.textContent = res.laya.choice;
    const confEl = document.getElementById('ai-conf');
    if (confEl) confEl.textContent = `confidence: ${(res.laya.confidence * 100).toFixed(0)}%`;
    // Latency
    const latEl = document.getElementById('ai-latency');
    if (latEl) latEl.textContent = `Laya ${res.laya.latency_ms}ms · total ${res.total_latency_ms}ms`;
    // Neuron labels with firing rate highlight
    const labelsEl = document.getElementById('neuron-labels');
    if (labelsEl && res.neural.firing_rates) {
      const names = ['wDL','wEL','wDR','wER','hL','hR','lFL','lML','lHL','lFR','lMR','lHR','CMD'];
      labelsEl.innerHTML = res.neural.firing_rates.map((r, i) =>
        `<span class="${r > 0.3 ? 'on' : ''}">${names[i] || i}</span>`
      ).join('');
    }
    // Raster visualization
    this._drawRaster(res.neural.raster, res.neural.firing_rates);
  }

  _drawRaster(raster, rates) {
    const canvas = document.getElementById('raster-canvas');
    if (!canvas || !raster || !raster.length) return;
    const ctx = canvas.getContext('2d');
    const T = raster.length, N = raster[0].length;
    canvas.width = 200; canvas.height = N * 8;
    ctx.fillStyle = '#1a1c22'; ctx.fillRect(0, 0, 200, canvas.height);
    const cw = 200 / T, ch = canvas.height / N;
    for (let t = 0; t < T; t++) {
      for (let n = 0; n < N; n++) {
        if (raster[t][n]) {
          const hue = n < 4 ? 45 : n < 6 ? 200 : n < 12 ? 120 : 280;
          ctx.fillStyle = `hsl(${hue}, 90%, 60%)`;
          ctx.fillRect(t * cw, n * ch, cw + 0.5, ch - 1);
        }
      }
    }
    // Labels
    const labels = [
      ...
      'leg-FL','leg-ML','leg-HL','leg-FR','leg-MR','leg-HR',
      'CMD'];
    ctx.font = '7px monospace'; ctx.fillStyle = '#9aa0a6';
    for (let n = 0; n < Math.min(N, labels.length); n++) {
      ctx.fillText(labels[n], 1, n * ch + ch - 1);
    }
  }
}
